<?php if (in_array('ANTI-EDAD', $lineas_pais)) { ?>
	<!-- ========================= Section 1 antiedad======================= -->
	<div class="section" id="antiedad" data-anchor="antiedad">

		<!-- ========================= antiedad Slide 1 [Portada principal]======================= -->
		<div class="slide" id="antiedad_slide1">
			<!-- contenido-->
		...
		foreach ($productos as $key => $value) {
			if ($value->linea == 'ANTI-EDAD' && $value->is_destacado == 1) {
				array_push($destacado, $value);
			} //end if
		} //end foreach
		?>
		<div class="slide" id="antiedad_slide2">
			<div class="container-fluid simpleCart_shelfItem">
				<div class="comprar-info-abajo-slide" style="">

					<a href="javascript:;" class="item_add"><button onclick="setTrazabilizad('<?php echo $service[0]->session_id; ?>','añadirCarrito','<?php echo str_replace("+", "X", $destacado[0]->nombre) . "-" . $destacado[0]->sku; ?>','<?php echo $destacado[0]->linea; ?>');this.disabled=true;" type="button" class="btn btn-primary bnt-lg comprar-info-btn">COMPRAR</button></a>
	 ...
	 ******************* -->
		...
		foreach ($productos as $key => $value) {
			...
			if ($value->linea == 'ANTI-EDAD' && strlen($value->orden_str) <= 3) {
				/******************************************
					...
					de la imagen.
						...
						<div class="comprar-info-arriba">
							...
							<?php if ($value->is_variable == 1) {
								...
								<!--------------------------------------------------------------------->
								<a><button onclick="selectproduct('select-<?php echo str_replace(' ', '-', $nombre_variable[0]); ?>');" type="button" class="btn btn-primary bnt-lg comprar-info-btn">COMPRAR</button></a>
								<!--------------------------------------------------------------------->
							<?php } else { ?>
								<!--------------------------------------------------------------------->
								<a href="javascript:;" class="item_add"><button onclick="setTrazabilizad('<?php echo $service[0]->session_id; ?>','añadirCarrito','<?php echo $value->nombre . "-" . $value->sku; ?>','<?php echo $value->linea; ?>');this.disabled=true;" type="button" class="btn btn-primary bnt-lg comprar-info-btn">COMPRAR</button></a>
								<!--------------------------------------------------------------------->
							<?php } //end else 
							?>
							<!--------------------------------------------------------------------->
		...
		?>
	</div><!-- end section antiedad --->
<?php } //end if in array
else {
	echo '<script>$("#liantiedad").hide();</script>';
} ?>