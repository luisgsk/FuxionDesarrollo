<?php if (in_array('LIMPIEZA', $lineas_pais)) { ?>
	<!-- ========================= Section 1 limpia======================= -->
	<div class="section" id="limpia" data-anchor="limpia">

		<!-- ========================= limpia Slide 1 [Portada principal]======================= -->
		<div class="slide" id="limpia_slide1">
			<!-- contenido-->
		...
		foreach ($productos as $key => $value) {
			if ($value->linea == 'LIMPIEZA' && $value->is_destacado == 1) {
				array_push($destacado, $value);
			} //end if
		} //end foreach
		?>
		<div class="slide" id="limpia_slide2">
			<div class="container-fluid simpleCart_shelfItem">
				<div class="comprar-info-abajo-slide" style="">

					<a href="javascript:;" class="item_add"><button onclick="setTrazabilizad('<?php echo $service[0]->session_id; ?>','añadirCarrito','<?php echo $destacado[0]->nombre . "-" . $destacado[0]->sku; ?>','<?php echo $destacado[0]->linea; ?>');this.disabled=true;" type="button" class="btn btn-primary bnt-lg comprar-info-btn"><?php if ($datos["pais"] == 'US' || $datos["pais"] == 'us') {
																																																																																						echo "BUY";
																																																																																					} else {
																																																																																						echo "COMPRAR";
																																																																																					} ?></button></a>
	 ...
	 ******************* -->
		...
		foreach ($productos as $key => $value) {
			...
			if ($value->linea == 'LIMPIEZA' && strlen($value->orden_str) <= 3) {
				/******************************************
		...
		?>
				...
				<div class="row simpleCart_shelfItem">
					...
					de la imagen.
						...
						<img data-src="<?php echo asset('images/productos/' . $datos["pais"] . '/' . $value->linea . '/' . $value->imagen . '') ?>" class="rounded mx-auto d-block img-fluid img-producto-arriba lazyload">
		...
		<?php
				/*************************************************
				Cierro el DIV slide cuado la cantidad de productos
				sea par (2 en 2)
				 *************************************************/
				if ($i % 2 == 0) {
					//echo "el $numero es par";
		...
		<!-- ========================= limpia Slide 3 ======================= -->
	</div><!-- end section limpia --->
<?php } //end if in array
else {
	echo '<script>$("#lilimpia").hide();</script>';
} ?>