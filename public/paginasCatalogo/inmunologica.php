<?php if(in_array('INMUNOLÓGICA', $lineas_pais)){ ?>
<!-- ========================= Section 1 inmunologica======================= -->
	<div class="section" id="inmunologica" data-anchor="inmunologica">
	
	<!-- ========================= inmunologica Slide 1 [Portada principal]======================= -->
	<div class="slide" id="inmunologica_slide1"><!-- contenido--></div><!-- end Slide 1 familia 1 -->

	<!-- ========================= inmunologica Slide 2 ======================= -->
	<?php $destacado = array();
	foreach ($productos as $key => $value) { 
		if($value->linea == 'INMUNOLÓGICA' && $value->is_destacado == 1){
			array_push($destacado, $value);
			}//end if
		}//end foreach
?>
	<div class="slide" id="inmunologica_slide2">
		<div class="container-fluid simpleCart_shelfItem">
			<div class="comprar-info-abajo-slide" style="">

						      <a href="javascript:;"  class="item_add"><button onclick="setTrazabilizad('<?php echo $service[0]->session_id; ?>','añadirCarrito','<?php echo str_replace("+","X",$destacado[0]->nombre)."-".$destacado[0]->sku; ?>','<?php echo $destacado[0]->linea;?>');this.disabled=true;" type="button" class="btn btn-primary bnt-lg comprar-info-btn"><?php if ($datos["pais"] == 'US' || $datos["pais"] == 'us'){echo"BUY";}else {echo"COMPRAR";} ?></button></a>
						    ...
						    <span style="display: none;" class="item_price">   <?php  echo $destacado[0]->precio; ?></span>
				 			...
				 			<span class="item_name" style="display: none;">	   <?php  echo $destacado[0]->nombre; ?></span>
						      ...
						      <img onclick="setTrazabilizad('<?php echo $service[0]->session_id; ?>','verInfo','<?php  echo $destacado[0]->nombre; ?>','<?php  echo $destacado[0]->linea; ?>');verInfo('<?php echo strtolower(str_replace(" ", "-", str_replace("+","X",str_replace("/","-",str_replace("&","",$destacado[0]->nombre)))));?>');"  src="<?php echo asset('images/icons/icon-info.png')?>" class="comprar-info-icon">
						  </div>
	 ...
	 ******************* -->
			...
			foreach ($productos as $key => $value) {
				...
				if($value->linea == 'INMUNOLÓGICA' && strlen($value->orden_str) <= 3){
				/******************************************
				 ...
				 <div class="franja-producto">
						<div class="row div-titulo-producto"><!-- row titulo-producto -->
							...
							<div class="col col-sm-2"></div><!-- cuadre de separacion izquierda -->
				 			...
				 			<div class="col-sm div-precio"><!-- col precio -->
				 			<?php $punto = strpos($value->precio, '.'); 														  ?>
				 			<span class="symbol-precio">	<?php echo  $value->simbolo?>
				 												 </span> 
				 			<span class="precio">			<?php if($punto != false){
				 													$precio = explode('.', $value->precio); echo $precio[0];
				 												}else {echo str_replace(",",".",number_format($value->precio));}
			...
			?>
				 ...
				 <div class="comprar-info-arriba">
				 	<!--------------------------------------------------------------------->
				 	<?php if($value->is_variable == 1) { $nombre_variable = explode('-', str_replace("+","X",str_replace("/","-",str_replace("&","",$value->nombre)))); ?>
				 	<!--------------------------------------------------------------------->
				 	<a><button onclick="selectproduct('select-<?php echo str_replace(' ','-',$nombre_variable[0]); ?>');" type="button" class="btn btn-primary bnt-lg comprar-info-btn"><?php if ($datos["pais"] == 'US' || $datos["pais"] == 'us'){echo"BUY";}else {echo"COMPRAR";} ?></button></a>	
				 	<!--------------------------------------------------------------------->
				 	<?php }else { ?>
				 	<!--------------------------------------------------------------------->
					<a href="javascript:;"  class="item_add"><button onclick="setTrazabilizad('<?php echo $service[0]->session_id; ?>','añadirCarrito','<?php echo $value->nombre."-".$value->sku;?>','<?php echo $value->linea; ?>');this.disabled=true;" type="button" class="btn btn-primary bnt-lg comprar-info-btn"><?php if ($datos["pais"] == 'US' || $datos["pais"] == 'us'){echo"BUY";}else {echo"COMPRAR";} ?></button></a>
					<!--------------------------------------------------------------------->
					<?php }//end else ?>
					<!--------------------------------------------------------------------->      
...
</div><!-- end section inmunologica --->
<?php }//end if in array
else {
	echo '<script>$("#liinmunologica").hide();</script>';
} ?>